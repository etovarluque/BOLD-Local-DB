...
def _generate_linux():
    _write_desktop_entry(
        os.path.join(PROJECT_ROOT, "1_Create_DB.desktop"),
        "1 - Create DB", "Build the BOLD local database",
        "./dev/bold_db_creator.sh", CREATE_DB_ICON_LINUX,
    )
    _write_desktop_entry(
        os.path.join(PROJECT_ROOT, "2_Open_web_viewer_BOLD_DB.desktop"),
        "2 - Open web viewer BOLD DB", "Open the BOLD DB web viewer",
        "./dev/launch_bold_db.sh", WEB_VIEWER_ICON_LINUX,
    )
    print("Linux shortcuts created at the project root:")
    print(f"  {PROJECT_ROOT}/1_Create_DB.desktop")
    print(f"  {PROJECT_ROOT}/2_Open_web_viewer_BOLD_DB.desktop")
...
def _generate_macos():
    ...
    # baked in), so making them work at the project root is just a copy +
    # chmod from the templates kept in dev/macos_shortcuts/. Custom Finder
    # icons for a plain .command file would require wrapping it as a real
    # .app bundle, which is out of scope here.
    src_dir = os.path.join(SCRIPT_DIR, "macos_shortcuts")
    for filename in ("1_Create_DB.command", "2_Open_web_viewer_BOLD_DB.command"):
        shutil.copyfile(os.path.join(src_dir, filename), os.path.join(PROJECT_ROOT, filename))
        os.chmod(os.path.join(PROJECT_ROOT, filename), 0o755)
    print("macOS shortcuts created at the project root:")
    print(f"  {PROJECT_ROOT}/1_Create_DB.command")
    print(f"  {PROJECT_ROOT}/2_Open_web_viewer_BOLD_DB.command")
